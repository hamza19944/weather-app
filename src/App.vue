...
export default {
  data(){
    return{
      api_key: "ff7744ec1831e648339ae5ea0d93763f",
  ...
  methods:{
    fetchMethod(e){
      if(e.key === "Enter"){
        fetch(`${this.url_api}weather?q=${this.query}&units=metric&appid=${this.api_key}`).then(res => res.json()).then(this.setResults)
        this.query = ''
      }
    },
    setResults(results){
